...
def init_database():
    ...
    try:
        ...
        # Add last_fruit cache column
        try:
            cur.execute('ALTER TABLE users ADD COLUMN IF NOT EXISTS last_fruit TEXT')
            logger.info("✅ 'last_fruit' cache column added/verified")

            # Backfill last_fruit from rolls table
            ...
            cur.execute('''
                ...
                WHERE last_fruit IS NULL
            ''')
            logger.info("✅ 'last_fruit' cache backfill complete")
        except Exception as e:
            logger.debug(f"Error handling last_fruit column: {e}")
...
def get_active_users_count() -> int:
    """Get total active users count efficiently"""
    conn = None
    try:
...
def get_total_rolls_count() -> int:
    """Get total rolls count efficiently"""
    conn = None
    try:
        ...
        count = cur.fetchone()[0]
        cur.close()
        logger.debug(f"✅ Total rolls count: {count}")
        return count
    except Exception as e:
...
def log_roll(user_id: int, username: str, fruit_name: str):
    ...
    try:
        ...
        # Update user
        logger.debug(f"📝 Updating user stats and last_fruit cache for {display_name} ({username})")
...
def get_users_with_last_fruit() -> List[Dict]:
    """Get all users with their last fruit roll (from cache) efficiently"""
    conn = None
    try:
        logger.debug("👥 Fetching users with last_fruit cache...")
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        cur.execute('''SELECT user_id,
                              username,
        ...
        rows = cur.fetchall()
        cur.close()
        logger.debug(f"✅ Fetched {len(rows)} users with cache")
        return [dict(row) for row in rows]
    except Exception as e:
...
async def on_ready():
    ...
    # Update active users and total rolls count
...
async def handle_stats(request):
    ...
    # Get all users with last fruit cache efficiently
    users = get_users_with_last_fruit()
    users_with_rolls = [u for u in users if u['next_roll_time']]
    ...
    html = STATS_PAGE.format(
        uptime=uptime,
        total_rolls=get_total_rolls_count(),
