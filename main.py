...
def get_user_count() -> int:
    """Get total number of users (efficient)"""
    try:
...
def get_total_roll_count() -> int:
    """Get total number of rolls (efficient)"""
    try:
        logger.debug("🎲 Getting total roll count")
        conn = get_db_connection()
        cur = conn.cursor()
...
def get_users_with_last_roll() -> List[Dict]:
    """Get users with their last roll in one query (fixes N+1 problem)"""
    try:
        logger.debug("👥 Fetching users with their last roll")
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        # Optimized query using LEFT JOIN LATERAL to get the most recent roll per user efficiently
        cur.execute('''
            SELECT u.user_id,
                   u.username,
                   u.total_rolls,
...
async def on_ready():
    ...
    # Update global statistics with efficient counts
    stats['active_users'] = get_user_count()
    stats['total_rolls'] = get_total_roll_count()
    logger.info(f"📊 Global statistics initialized:")
    logger.info(f"   - Active Users: {stats['active_users']}")
    logger.info(f"   - Total Rolls: {stats['total_rolls']}")
...
async def handle_stats(request):
    ...
    # Get all users with their last roll in one efficient query (fixes N+1 problem)
    ...
    users_html = "".join(users_html_list) if users_html_list else "<p style='text-align: center; opacity: 0.7;'>No users have logged rolls yet</p>"
