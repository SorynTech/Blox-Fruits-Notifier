...
def init_database():
    ...
    try:
        ...
        # Backfill last_fruit column from rolls table (performance optimization)
        # Bolt ⚡: This avoids N+1 queries when displaying the stats dashboard
        logger.info("🔄 Backfilling 'last_fruit' column...")
        cur.execute('''
            UPDATE users u
        ...
        ''')
        logger.info("✅ 'last_fruit' column backfilled")
...
def create_or_update_user(user_id: int, username: str):
    ...
    try:
        ...
        if existing:
            ...
            # Bolt ⚡: Update active users counter in memory
...
def log_roll(user_id: int, username: str, fruit_name: str):
    ...
    try:
        ...
        # Update user
        # Bolt ⚡: Including last_fruit in the update to maintain the cache
...
def get_all_users() -> List[Dict]:
    ...
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        # Bolt ⚡: Include last_fruit cache to eliminate N+1 queries in the dashboard
...
def get_active_users_count() -> int:
    """Get count of active users from database (optimization)"""
    # Bolt ⚡: Much faster than fetching all users and using len()
    conn = None
    try:
...
def get_total_rolls_count() -> int:
    """Get total count of rolls from database (optimization)"""
    # Bolt ⚡: Optimized to get total rolls count efficiently
    conn = None
    try:
...
def get_users_to_notify(now: datetime) -> List[Dict]:
    """Get only users who need to be notified (optimization)"""
    # Bolt ⚡: Fetches only the necessary subset of users instead of the entire table
    conn = None
    try:
        logger.debug(f"⏰ Fetching users to notify (time <= {now})")
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        cur.execute('''SELECT user_id, username, next_roll_time
                       FROM users
...
async def on_ready():
    ...
    # Initialize total rolls from database
    ...
    # Bolt ⚡: Using optimized count query instead of fetching all users
...
async def notification_checker():
    ...
    # Bolt ⚡: Optimized to fetch only users who need notification
    users_to_notify = get_users_to_notify(now)
...
async def handle_stats(request):
    ...
    for user in users_sorted:
        ...
        # Bolt ⚡: Using cached last_fruit from the user record instead of get_user_rolls()
        # This eliminates N database queries (O(1) instead of O(N))
