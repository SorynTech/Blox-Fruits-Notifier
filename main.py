...
def get_users_with_last_fruit() -> List[Dict]:
    """Get all users and their latest rolled fruit in a single optimized query (Avoids N+1)"""
    conn = None
    try:
        logger.debug("👥 Fetching users with their latest fruit (Optimized)")
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        # Using LEFT JOIN LATERAL to fetch the latest roll for each user efficiently
        cur.execute('''
            SELECT u.user_id,
                   u.username,
                   u.total_rolls,
        ...
        rows = cur.fetchall()
        cur.close()
        logger.debug(f"✅ Fetched {len(rows)} users with fruit data")
        return [dict(row) for row in rows]
    except Exception as e:
...
async def handle_stats(request):
    ...
    # Get all users and their last fruit in one optimized query
    ...
    # Build users list HTML using "".join() for better performance O(N)
    users_html = "".join([
        f"""
        <div class="user-item">
            <div class="user-info">
                ...
                <div class="user-stats">
                    Last Roll: {user['last_fruit'] or "None"} | Total: {user['total_rolls']} | {"🔔 Enabled" if user['notifications_enabled'] else "🔕 Disabled"}
                </div>
            </div>
            <div class="next-roll">
