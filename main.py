...
def check_auth(request) -> bool:
    """Check HTTP Basic Auth with timing attack protection"""
    ...
    try:
        ...
        # This prevents an attacker from guessing the credentials byte by byte
...
async def handle_stats(request):
    ...
    for user in users_sorted:
        ...
        safe_fruit = html.escape(str(last_fruit))
    ...
    try:
        ...
        if suspended_users:
            ...
            for user in suspended_users:
                ...
                # Sanitize user-controlled data to prevent XSS
                safe_username = html.escape(str(user['username']))
                reason_text = user.get('suspension_reason')
                safe_reason = html.escape(str(reason_text)) if reason_text else 'No reason provided'
