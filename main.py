...
def init_database():
    ...
    try:
        ...
        # Backfill last_fruit from rolls table if it's null
        ...
        logger.critical(f"❌ Supabase initialization error: {e}")
        if conn:
            conn.rollback()
        raise
    finally:
...
def get_total_rolls_count() -> int:
    """Get total number of rolls from the rolls table"""
    conn = None
    try:
...
def get_active_users_count() -> int:
    """Get count of active users from users table"""
    conn = None
    try:
...
def get_users_to_notify(now: datetime) -> List[Dict]:
    """Get users who need a roll notification"""
    conn = None
    try:
        logger.debug(f"⏰ Fetching users to notify at {now}")
        conn = get_db_connection()
...
async def handle_stats(request):
    ...
    for user in users_sorted:
        ...
        # Get their last fruit from cache
