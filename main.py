...
def get_user_count() -> int:
    """Get total number of users (optimized)"""
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor()
...
def get_total_roll_count() -> int:
    """Get total number of rolls (optimized)"""
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor()
...
def get_users_with_last_roll() -> List[Dict]:
    """Get all users with their most recent roll in one query (prevents N+1)"""
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        # Using LEFT JOIN LATERAL for optimal performance to get latest roll per user in a single batch
        cur.execute('''
...
async def on_ready():
    ...
    # Update stats from database (optimized)
    stats['active_users'] = get_user_count()
    stats['total_rolls'] = get_total_roll_count()
    logger.info(f"📊 Stats initialized: {stats['active_users']} users, {stats['total_rolls']} total rolls")
...
async def handle_stats(request):
    ...
    # Get all users with their last fruit roll (Optimized single-query batch retrieval)
    users = get_users_with_last_roll()

    # Sort users by next roll time (upcoming first)
    users_sorted = sorted(
        [u for u in users if u['next_roll_time']],
        key=lambda x: x['next_roll_time']
    )

    # Build users list HTML using list comprehension and join for O(N) performance
    ...
    users_html = "".join([create_user_item(u) for u in users_sorted])
