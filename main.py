...
def get_total_rolls_count() -> int:
    """Get total count of all rolls in the database"""
    conn = None
    try:
...
def get_users_with_last_fruit() -> List[Dict]:
    ...
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        # Use a LATERAL JOIN to get the latest roll for each user efficiently
        cur.execute('''
...
def get_users_to_notify(now: datetime) -> List[Dict]:
    """Get all users who need a roll reminder notification"""
    conn = None
    try:
...
async def notification_checker():
    ...
    now = datetime.now(timezone.utc)
    # Get only users who need to be notified (efficient DB-level filtering)
...
async def handle_stats(request):
    ...
    # Get all users with their last fruit in a single query (solves N+1 problem)
